...
function startIntroHtml(variant=variantForState()){
 const bagText = variant==='short' ? '' : '<p>V batohu najdete také prázdný pytel. Dávejte do něj předměty, které už jste ve hře použili. Nebudou se vám plést s těmi, které teprve přijdou na řadu, a nic se vám po cestě neztratí.</p>';
 return `<div class="hero hero-intro start-copy"><p class="hero-subtitle">Venkovní úniková hra v historickém srdci Plzně.</p><p class="lead">Celou hrou vás povede aplikace. Dívejte se pozorně, poslouchejte a používejte vše, co po cestě získáte.</p><p>Některé předměty využijete hned, na jiné přijde čas až později. Na každé zastávce si nejdříve najděte zámek, který budete otevírat. Každý je označený kartičkou s číslem zastávky, takže předem poznáte, zda hledáte trojmístný nebo čtyřmístný kód, případně slovo.</p>${bagText}<p>Pokud se zaseknete, znovu si pomalu a pozorně přečtěte zadání v aplikaci. Cesta dál je v něm vždy ukrytá. Když to nepomůže, využijte nápovědy. U každé zastávky jsou tři a postupně vás navedou. Pokud budete potřebovat větší pomoc, najdete v nabídce i řešení.</p><p>Kdyby nastal technický nebo jiný problém, použijte tlačítko SOS v menu aplikace. Nastartujte mozkové závity a užijte si hru.</p></div>`;
}
...
function adminPreviewStart(){
 modal(`<h2>Úvodní stránka</h2><p class="small muted">Náhled pro admina, nemění rozehranou hru žádného týmu.</p>
  ...
  ${startIntroHtml(adminPreviewVariant())}
...
function openSelfieBooth(){
 ...
 modal(`<h2>Památeční fotka</h2><p class="small muted">Povolte fotoaparát a postavte se do okénka v rámečku. Fotka zůstane ve vašem telefonu, dokud ji sami nesdílíte nebo nestáhnete.</p>
  <div class="selfie-mode-tabs" role="group" aria-label="Vyberte styl fotky">
   <button id="selfieModeFrame" class="selfie-mode-btn active" type="button" onclick="setSelfieMode('frame')">Rámeček</button>
   <button id="selfieModeGroll" class="selfie-mode-btn" type="button" onclick="setSelfieMode('groll')">S Grollem</button>
  </div>
  <div class="selfie-stage" id="selfieStage">
   ...
   <img id="selfieGrollBorder" class="selfie-groll-border" src="${GROLL_SELFIE_FRAME_SRC}" alt="" aria-hidden="true">
...
function setSelfieMode(mode){
 ...
 const isGroll=selfieMode==='groll';
 stage?.classList.toggle('selfie-mode-groll', isGroll);
 stage?.classList.toggle('selfie-mode-frame', !isGroll);
...
async function captureGrollSelfie(){
 ...
 canvas.height=selfieMode==='frame' ? (frame.naturalHeight || frame.height || SELFIE_CANVAS_SIZE.h) : SELFIE_CANVAS_SIZE.h;
 const ctx=canvas.getContext('2d');
 ...
 if(selfieMode==='groll'){
  const frameOverlay=await loadImage(GROLL_SELFIE_FRAME_SRC);
 ...
 if(result && selfieLastBlob){
  ...
  const borderEl=$('#selfieGrollBorder');
...
function scaledSelfieFrameWindow(canvas){
 ...
 const sx=canvas.width/SELFIE_CANVAS_SIZE.w;
 const sy=canvas.height/SELFIE_CANVAS_SIZE.h;
 return {
  x:win.x*sx,
  y:win.y*sy,
  w:win.w*sx,
...
function drawAntiqueCameraWindow(ctx, source, rect){
 const off=document.createElement('canvas');
 off.width=Math.max(1,Math.round(rect.w));
 off.height=Math.max(1,Math.round(rect.h));
 const octx=off.getContext('2d');
 octx.fillStyle='#1b0f07';
 octx.fillRect(0,0,off.width,off.height);
 octx.save();
 octx.filter='contrast(1.02) brightness(.98) saturate(.92)';
 drawMirroredCover(octx, source, 0, 0, off.width, off.height);
 octx.restore();
 applyAntiquePhoto(octx, off);
 drawSelfieFinish(octx, off);
 ctx.save();
 ctx.drawImage(off,rect.x,rect.y,rect.w,rect.h);
 ctx.globalCompositeOperation='multiply';
 ctx.strokeStyle='rgba(43,24,10,.28)';
...
function drawGrollOverlay(ctx, canvas, groll){
 ...
 const octx=off.getContext('2d');
 octx.drawImage(groll,0,0,off.width,off.height);
 applyAntiquePhoto(octx, off);
 ctx.save();
 ctx.shadowColor='rgba(20,10,3,.45)';
 ctx.shadowBlur=18;
 ctx.shadowOffsetY=8;
...
function retakeGrollSelfie(){
 ...
 const borderEl=$('#selfieGrollBorder');
 if(borderEl) borderEl.style.display='';
