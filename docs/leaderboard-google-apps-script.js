...
const HEADERS = {
  ...
  events: ['time','timeCz','teamId','team','accessCode','type','eventName','station','stationTitle','hint','value','detail'],
  leaderboard: ['id','team','total','hints','solutions','title','date','dateCz']
};
...
function addLeaderboard_(e){ const id=String(e.parameter.id||''); if(!id) return json_({ok:false,error:'missing id'},e); const sh=getSheet_(SHEETS.leaderboard,HEADERS.leaderboard); const date=String(e.parameter.date||new Date().toISOString()); if(findRowById_(sh,id)<0) sh.appendRow([id,String(e.parameter.team||'Bez nazvu'),Number(e.parameter.total||0),Number(e.parameter.hints||0),Number(e.parameter.solutions||0),String(e.parameter.title||''),date,formatDateTimeCz_(date)]); return json_({ok:true},e); }
...
function leaderboardRows_(){ return rows_(SHEETS.leaderboard).filter(r=>r.id).map(r=>({id:String(r.id),team:String(r.team||'Bez nazvu'),total:Number(r.total||0),hints:Number(r.hints||0),solutions:Number(r.solutions||0),title:String(r.title||''),date:String(r.date||''),dateCz:String(r.dateCz||'')})).sort((a,b)=>a.total-b.total).slice(0,50); }
